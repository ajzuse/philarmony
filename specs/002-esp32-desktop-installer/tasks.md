...
**Purpose**: Close residual gaps after Phase 10 tasks were marked complete (triple-validated post-implement reassessment 2026-08-04). Manual stages remain in `specs/005-manual-validation` (VS-1/VS-4/VS-5) — not listed here.

- [x] T098 CRITICAL Install PlatformIO (or cache prebuilt firmware artifacts) and run `make sync-installer-firmware` on Linux **and** Windows/macOS release jobs in `.github/workflows/desktop-installer-release.yml` so host installers embed `firmware.bin`/`bootloader.bin`/`partitions.bin` (assets currently README-only) per FR-012 / plan:CI (missing)
- [x] T099 CRITICAL Codesign the macOS `.app` **before** `build_dmg.sh` creates the DMG, then notarize/staple that signed DMG in `apps/esp32-desktop-installer/packaging/macos/` + `.github/workflows/desktop-installer-release.yml` (current `notarize.sh` signs the app after the DMG already copied an unsigned bundle) per FR-012 (contradicts)
- [x] T100 CRITICAL Implement real RPM `%install`/`%files` payload in `apps/esp32-desktop-installer/packaging/linux/rpm/philarmony-installer.spec` and make `build_rpm.sh` hard-fail when `rpmbuild` is missing (no exit-0 tar fallback) per FR-012 (missing)
- [x] T101 Always clear WiFi password on JSON import (including non-`***` plaintext) and force re-entry before flash in `packages/philarmony_core/lib/src/serialization/profile_store.dart` + tests per FR-009 (contradicts)
- [x] T102 Require AppImage **and** `.deb` **and** `.rpm` artifacts in `.github/workflows/desktop-installer-release.yml` Linux package gate (replace `find … | head -1` one-of check) per FR-012 (partial)
- [x] T103 Hard-fail `packaging/linux/scripts/build_appimage.sh` and `build_deb.sh` when `appimagetool`/`dpkg-deb` are missing (no exit-0 staging-tar success path) per FR-012 / plan:CI (contradicts)
- [x] T104 Inject advanced custom sensor JSON `parameters` (temp + humidity) into Preferences NVS via `packages/philarmony_core/lib/src/mapping/nvs_config_mapper.dart` `toPreferencesStringEntries` (today only `toHardwareJson` carries them) per FR-003 (partial)
- [x] T105 Add WebSocket soft post-flash verify path (in addition to HTTP `/api/info`) in `apps/esp32-desktop-installer/lib/flash/esptool_firmware_flasher.dart` `SoftPostFlashVerifier` per FR-008 / contracts/flash-pipeline.md (partial)
- [x] T106 Populate `SerialPortInfo.vid`/`pid` from `flutter_libserialport` and surface CP210x/CH340/FTDI USB-UART hints in `apps/esp32-desktop-installer/lib/device/serial_device_detector.dart` + Device step per FR-001 / contracts/flash-pipeline.md (missing)
- [x] T107 [P] Externalize remaining hardcoded wizard/UI strings (review labels, flash log Verbose/Quiet/Export, pins legend, device unsupported/partition picker, profile_io snackbars, home language tooltip) into `apps/esp32-desktop-installer/lib/l10n/*.arb` per FR-010 (partial)
- [x] T108 Add `FlashJobState.preflight` and emit stage-accurate progress from esptool stdout/stderr (stop labeling all stream lines as `writingApp`) in `packages/philarmony_core/lib/src/models/flash_job.dart` + `apps/esp32-desktop-installer/lib/flash/esptool_firmware_flasher.dart` per contracts/flash-pipeline.md (partial)
- [x] T109 Validate static IP fields (ip/gateway/netmask/dns) before WiFi Next and align flashed wifi NVS `static_ip` with firmware-consumable `WifiConfig` (today firmware ignores it) in `wifi_step.dart` + `nvs_config_mapper.dart` (+ firmware `ConfigManager` if required) per FR-007 (partial)
- [x] T110 [P] Enforce `EspGpioMap.usableGpios` (reject out-of-range / non-usable GPIOs) in `packages/philarmony_core/lib/src/validation/pin_validator.dart` per FR-004 (partial)
- [x] T111 Add ESP-IDF/Preferences-compatible NVS binary round-trip (or Espressif `nvs_partition_gen` parity) test beyond header smoke in `packages/philarmony_core/test/` per FR-008 / plan:Testing (partial)
- [x] T112 Require non-empty WiFi password on WiFi step `validateCurrentStep` before Next in `apps/esp32-desktop-installer/lib/wizard/installer_session_controller.dart` per FR-007 / FR-009 (partial)
- [x] T113 [P] Add optional nearby WiFi SSID scan UI on `apps/esp32-desktop-installer/lib/wizard/steps/wifi_step.dart` per FR-007 (missing)
- [x] T114 [P] Add GPLv3 license headers to hand-written Dart tests under `apps/esp32-desktop-installer/test/` and `packages/philarmony_core/test/` (skip generated `lib/l10n/`) per Constitution GPLv3 (partial)
- [x] T115 [P] Fix invalid Inno Setup `AppId` GUID in `apps/esp32-desktop-installer/packaging/windows/inno/philarmony-installer.iss` per FR-012 optional Inno (partial)
